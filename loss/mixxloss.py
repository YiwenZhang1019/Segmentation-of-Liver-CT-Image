...
class MS_SSIM(torch.nn.Module):
    def __init__(self, window_size = 11,size_average = True,alpha=0.25,gamma=2,reduction='mean'):
    ...
    def forward(self, img1, img2, levels=5):
        ...
        for i in range(levels): #5 levels
            ...
            img2 = filtered_im2
        logits = img1.float()  # use fp32 if logits is fp16
        with torch.no_grad():
            alpha = torch.empty_like(logits).fill_(1 - self.alpha)
            alpha[img2 == 1] = self.alpha
        ce_loss = (-(img2 * torch.log(logits)) - (
                (1 - img2) * torch.log(1 - logits)))
        # ce_loss=(-(label * torch.log(torch.softmax(logits, dim=1))) - (
        #             (1 - label) * torch.log(1 - torch.softmax(logits, dim=1))))
        pt = torch.where(img2 == 1, logits, 1 - logits)
        ...
        loss_ms=torch.prod((msssim[levels-1]**weight[levels-1] * mcs[0:levels-1]**weight[0:levels-1]))
        loss=0.3*loss_ms+0.7*loss_fc
        return loss